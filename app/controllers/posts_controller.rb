class PostsController < ApplicationController
  def index
    # This action retrieves all posts belonging to a specific user (specified by params[:user_id]) and assigns them to the @posts instance variable.
  ...
  def show
    # This action finds a single post based on the id parameter in the request and assigns it to the @post instance variable.
  ...
  def create
    ...
    # It builds a new post associated with the current user using the parameters from the form (permitted by post_params).
    # It sets the initial values for comments_counter and likes_counter (assuming these are counters for comments and likes on the post).
  ...
  def post_params
    # This is a private method that uses strong parameters to whitelist and permit only the :title and :text parameters for creating a post. It helps protect against mass assignment vulnerabilities.
